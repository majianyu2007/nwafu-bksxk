...
- **System notifications** — grab success, seat-available, and monitor auto-stop, with course + class + time. Desktop + mobile.
- **Conflict-aware** — selecting a class the server marks as conflicting warns you (naming the conflict) *before* submitting. Capacity/余量 is always labeled a non-realtime snapshot with its as-of time.
...
```bash
flutter pub get
flutter test          # crypto vectors + param builders + monitor engine (30 tests)
