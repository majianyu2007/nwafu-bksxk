...
(function (BH_UTILS, undefined) {
    ...
    BH_UTILS.showScrollbar = function(){
        ...
        //qiyu 2016-12-21 隐藏滚动条时头部也需要留白，避免移动
        $('[bh-header-role="bhHeaderMini"]').css('padding-right', _originalBodyPad);
        $('[bh-header-role="bhHeader"]').css('padding-right', _originalBodyPad);
    }
...
;(function (_content) {
    ...
    _content.set = function($setContainer) {
        setContentMinHeight($setContainer);
    }
    ...
    function setContentMinHeight($setContainer) {
        if ($setContainer && $setContainer.length > 0) {
            var $window = $(window);
            var windowHeight = $window.height();
            var footerHeight = $(".cv-page-footer").outerHeight();
            var headerHeight = $(".cv-page-header").outerHeight();
            var minHeight = windowHeight - headerHeight - footerHeight - 1;
            $setContainer.css("min-height", minHeight + "px");
        }
    };

})(window.CVContentMinHeight = window.CVContentMinHeight || {});
